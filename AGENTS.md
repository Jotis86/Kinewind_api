...
- **Calendario semanal** (`GET /calendario/`, item "Calendario" en el menú): vista de lunes a domingo tipo Google Calendar con el rango fijo **10:00–22:00** (`SLOT_START`/`SLOT_END` de `services.py`, 1 min = 1 px). Navega por semanas y meses con `?week_start=YYYY-MM-DD` (se snapea al lunes). Excluye citas canceladas; cada bloque enlaza a la edición de la cita. Posiciones calculadas en `CalendarView`.
